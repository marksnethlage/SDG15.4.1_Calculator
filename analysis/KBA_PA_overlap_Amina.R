...
folder <- getwd()
finfolder <- paste0(folder, "/files_country_", year_run) #folder where the files per country will be saved
...
kbas <- st_read(dsn = paste0(getwd(), '/data/clipped/KBA2020.shp'), stringsAsFactors = F) #note field called Shape not geometry #nonclipped is KBAsGlobal_2020_September_02_POL.shp
kbas <- kbas[!is.na(kbas$SitRecID),] #remove any NAs
pas <- st_read(dsn = paste0(getwd(), '/data/clipped/WDPA2020.shp') , stringsAsFactors = F) #nonclipped is WDPA_May2021_Public_shp/WDPA_May2021_Public_shp-polygons.shp
